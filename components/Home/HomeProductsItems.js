...
const HomeProductsItems = ({ products, productsDetails, showCategories }) => {
  ...
  const changeBgColor = (index) => {
    setActive(index);
    pageNumber[index] && setBackground("bg-black text-white");
  };
        ...
        {pageNumber.map((number, i) => {
          return (
            <li
              ...
              className={` ${active === i && background} ${
                (number === 1) & (displayedProducts[0].title === "iPhone 9") &&
                "bg-black text-white"
              } w-10 h-10 font-semibold border border-black rounded flex justify-center items-center cursor-pointer`}
            >
              {number}
            </li>
          );
